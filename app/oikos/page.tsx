...
export default function OikosPage() {
      ...
      const response = await fetch("/api/memoria", {
        ...
        body: JSON.stringify({
          ...
          concept: {
            ...
            micro_question: item.micro_test_q || `What is the key insight from: ${item.title}?`,
            micro_answer: item.micro_test_a || item.content,
            source: item.type === "tiktok" ? "tiktok" : 
                   item.type === "book_summary" ? "book_summary" :
                   item.type === "book_micro" ? "book" : "puzzle"
          }
        })
      });
  ...
  const getTypeLabel = (type: string) => {
    switch (type) {
      case "tiktok": return "MICRO-INSIGHT";
      case "book_summary": return "BOOK SUMMARY";
      case "book_micro": return "BOOK LESSON";
      ...
      <>
        <Main>
          ...
          <BrutalistCard borderWidth="1.5" className="p-6">
            <div className="font-mono text-sm text-muted-foreground">Loading today's feed...</div>
          </BrutalistCard>
        </Main>
    ...
    <>
      <Main>
        ...
        <div className="space-y-6">
          {feedItems.length === 0 ? (
            <BrutalistCard borderWidth="1.5" className="p-6">
              <div className="font-mono text-sm text-muted-foreground">No feed items available.</div>
            </BrutalistCard>
          ) : (
            feedItems.map((item, index) => (
              <BrutalistCard key={item.id} borderWidth={index === 0 ? "1.5" : "1"} className="p-6">
                ...
                <div className="font-mono text-sm space-y-4">
                  ...
                  {item.example && (
                    <div className="border-t border-border pt-4">
                      <div className="font-serif text-sm mb-2 engraved">Example</div>
                      <p className="text-muted-foreground italic text-xs">{item.example}</p>
                    </div>
                  )}
                  ...
                  {item.micro_test_q && (
                    <div className="border-t border-border pt-4">
                      ...
                      <div className="space-y-3">
                        <p className="text-muted-foreground">{item.micro_test_q}</p>
                        
                        {revealedAnswers.has(item.id) ? (
                          <div>
                            <div className="mb-2 font-semibold">Answer:</div>
                            <p className="text-muted-foreground mb-3">{item.micro_test_a}</p>
        ...
        <BrutalistCard borderWidth="1" className="p-4 mb-4">
          ...
          <div className="font-mono text-xs text-muted-foreground mt-2 space-y-1">
            {feedItems.map((item, idx) => (
              <div key={item.id}>{idx + 1}. {getTypeLabel(item.type)}</div>
            ))}
          </div>
        </BrutalistCard>
